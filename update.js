...
async function actualizarConApiOficial() {
  ...
  console.log("Procesando y generando datos estáticos limpios...");
  ...
  try {
    ...
    // Función interna para mapear el escudo directamente en el servidor
    ...
    for (const match of partidosDeLaFecha) {
      ...
      if (localNombre && visitaNombre) {
        // Ya guardamos la URL completa y lista para usar directo en la etiqueta <img>
        partidosArray.push({
          local: localNombre,
          archivoLocal: CDN_URL + mapEscudo(localNombre),
          golesLocal: Number(golesL),
          visitante: visitaNombre,
          archivoVisitante: CDN_URL + mapEscudo(visitaNombre),
